  ...
  return (
    <div className="card p-5 bg-white rounded-2xl border border-slate-100 shadow-sm">
        ...
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-4">
            ...
            <div className="flex flex-1 items-center justify-between gap-2">
              <div>
                <p className="font-medium text-[#07245e] line-clamp-1">{item.name}</p>
      ...
      <div className="mt-6 border-t border-slate-100 pt-4 space-y-2">
        ...
        <div className="flex items-center justify-between border-t border-slate-100 pt-2 text-lg font-bold text-[#07245e]">
          <span>Total</span>
          <span>{formatPrice(subtotal)}</span>
        </div>
      </div>
